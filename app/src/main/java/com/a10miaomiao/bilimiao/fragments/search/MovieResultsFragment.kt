...
import com.a10miaomiao.bilimiao.adapter.search.MovieResultsAdapter
 ...
 */
class MovieResultsFragment: BaseFragment(){
    ...
    override fun loadData() {
        ...
        MiaoHttp.newStringClient(
                url = BiliApiService.getSearchMovie(keyword,pageNum,pageSize),
                onResponse = {
                    isNullList = false
                    hideProgressBar()
                    try {
                        var dataBean = Gson().fromJson(it, SearchMovieInfo::class.java)
                        if (dataBean.code != 0) {
    ...
    companion object {
        fun newInstance(keyword: String): MovieResultsFragment {
            val fragment = MovieResultsFragment()
            val bundle = Bundle()
